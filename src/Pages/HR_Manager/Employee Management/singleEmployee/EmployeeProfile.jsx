...
function EmployeeProfile({ employeeData, role }) {
  ...
  const [termReason, setTermReason] = React.useState("");
  const [termDate, setTermDate] = React.useState("");
  const [isProcessing, setIsProcessing] = React.useState(false);

  const handleTerminate = async () => {
    if (!termReason) return alert("Reason is required");
    try {
      setIsProcessing(true);
      await axiosPrivate.post(`/employees/${employeeData?.id}/terminate/`, {
        termination_reason: termReason,
        last_working_date: termDate || null
      });
    ...
    <div id="left" className="flex w-full flex-col h-full p-6 gap-6 transition-all overflow-y-auto scrollbar-hidden">
      ...
      <div id="top" className="items-center justify-center flex flex-col gap-4 shrink-0">
        ...
        <div className="flex items-center justify-center">
          <p className={`font-bold px-4 py-1.5 text-[10px] uppercase tracking-widest rounded-full shadow-xs border ${employeeData?.payroll?.employeestatus === "Active"
          ...
          {role === "Manager" && (
            <>
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-orange-50 hover:bg-orange-100 text-orange-600 transition-all border border-orange-100 active:scale-[0.98]"
              >
                <p className="text-[10px] font-bold uppercase tracking-widest">Terminate</p>
                <Icon className='w-4 h-4' name={'UserMinus'} />
              </button>
      ...
      {showTermModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6">
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-4">Terminate Employee</h3>
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Reason</label>
                  <textarea
                    className="w-full p-2 border rounded-lg text-sm dark:bg-slate-900 dark:border-slate-700"
                    rows="3"
                    value={termReason}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Last Working Date</label>
                  <input
                    type="date"
                    className="w-full p-2 border rounded-lg text-sm dark:bg-slate-900 dark:border-slate-700"
                    value={termDate}
              ...
              <div className="flex gap-3 mt-6 justify-end">
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm font-semibold bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition shadow-lg shadow-orange-500/20"
                >
