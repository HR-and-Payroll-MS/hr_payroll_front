...
const drawer= <>{isModalOpen && (
  <FileDrawer width="w-2/3" isModalOpen={isModalOpen} closeModal={closeEditor}>
      ...
      <div className="bg-slate-900 dark:bg-slate-700 p-4 sticky z-20 top-0 flex justify-between items-center shadow-md">
        <div>
          <h1 className="text-white font-bold flex items-center gap-2">
            ...
            {selectedCodeId ? `New Version: ${taxData.version}` : 'Configure New Tax Code'}
          </h1>
        </div>
      ...
      <div className="p-6 flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-50 dark:bg-slate-900/40 p-5 rounded-xl border border-slate-100 dark:border-slate-700">
          {!selectedCodeId && (
            <div className="col-span-1 md:col-span-1">
              <label className="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Tax Name</label>
              <InputField onSelect={(e) => handleChange('name', e)} icon={false} placeholder='e.g. Federal Tax' searchMode='input' maxWidth='w-full'/>
            </div>
          )}
          <div className="w-20">
            <label className="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Version</label>
          ...
          <div className="flex-1">
            <label className="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Valid From</label>
            <input type="date" value={taxData.validFrom} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 p-2 rounded-lg text-sm dark:text-white outline-none focus:ring-2 ring-blue-500/20" onChange={(e) => handleChange('validFrom', e.target.value)} />
          </div>
          <div className="flex-1">
            <label className="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Valid To (Optional)</label>
            <input type="date" value={taxData.validTo || ''} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 p-2 rounded-lg text-sm dark:text-white outline-none focus:ring-2 ring-blue-500/20" onChange={(e) => handleChange('validTo', e.target.value || null)} />
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-2">
            <p className="text-[11px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest">Calculation Method</p>
            <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-xl shadow-inner">
              <button onClick={() => handleChange('incomeTax.type', 'progressive')} className={`px-6 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${taxData.incomeTax.type === 'progressive' ? 'bg-white dark:bg-slate-700 shadow text-blue-600 dark:text-blue-400' : 'text-slate-400'}`}>Progressive</button>
              <button onClick={() => handleChange('incomeTax.type', 'flat')} className={`px-6 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${taxData.incomeTax.type === 'flat' ? 'bg-white dark:bg-slate-700 shadow text-blue-600 dark:text-blue-400' : 'text-slate-400'}`}>Flat Rate</button>
            </div>
          </div>
          ...
          {taxData.incomeTax.type === 'flat' ? (
            <div className="bg-blue-50 dark:bg-blue-900/10 p-6 rounded-xl border border-blue-100 dark:border-blue-800/30 flex items-center gap-4">
              <label className="text-sm font-bold text-slate-700 dark:text-slate-300">Set Flat Rate Percentage:</label>
              <div className="relative">
                <input type="number" value={taxData.incomeTax.flatRate} className="w-28 outline-none p-2.5 rounded-lg border-2 border-blue-200 dark:border-blue-800 bg-white dark:bg-slate-900 font-black text-blue-600 text-lg" onChange={(e) => handleChange('incomeTax.flatRate', parseInt(e.target.value))} />
                <span className="absolute right-3 top-3 font-bold text-blue-300">%</span>
              </div>
            </div>
          ) : (
            <div className="space-y-3">
              <SectionHeader title="Income Brackets" onAdd={() => {
              ...
              <div className="grid grid-cols-1 gap-2">
                {taxData.incomeTax.brackets.map((bracket, index) => (
                  <div key={index} className="flex gap-3 items-end bg-slate-50 dark:bg-slate-900/40 p-4 rounded-lg border border-slate-100 dark:border-slate-700 transition-all hover:border-blue-200 dark:hover:border-blue-900">
                    <div className="flex-1">
                      <label className="text-[9px] text-slate-400 font-bold uppercase mb-1 block">Min Amount</label>
                      <input type="number" value={bracket.min || ''} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 p-2 rounded text-sm font-bold" onChange={(e) => { /* update logic */ }} />
                    </div>
                    <div className="flex-1">
                      <label className="text-[9px] text-slate-400 font-bold uppercase mb-1 block">Max (Empty for ∞)</label>
                      <input type="number" value={bracket.max || ''} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 p-2 rounded text-sm font-bold" onChange={(e) => { /* update logic */ }} />
                    </div>
                    <div className="w-24">
                      <label className="text-[9px] text-slate-400 font-bold uppercase mb-1 block">Rate %</label>
                      <input type="number" value={bracket.rate} className="w-full bg-white dark:bg-slate-800 border-2 border-blue-100 dark:border-blue-900 p-2 rounded text-sm font-black text-blue-600 dark:text-blue-400" onChange={(e) => { /* update logic */ }} />
                    </div>
                    {index > 0 && (
                      <button onClick={() => removeItem('incomeTax.brackets', index)} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all">
                        <Trash2 size={18} />
                      </button>
                    )}
                  </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-slate-50 dark:bg-slate-900/20 p-5 rounded-xl border border-slate-100 dark:border-slate-700">
            <h2 className="text-[11px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest mb-4">Pension Contribution</h2>
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="text-[10px] font-bold text-slate-500 mb-1 block">Employee %</label>
                <input type="number" value={taxData.pension.employeePercent} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 p-2.5 rounded-lg font-bold" onChange={(e) => handleChange('pension.employeePercent', parseInt(e.target.value))} />
              </div>
              <div className="flex-1">
                <label className="text-[10px] font-bold text-slate-500 mb-1 block">Employer %</label>
                <input type="number" value={taxData.pension.employerPercent} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 p-2.5 rounded-lg font-bold" onChange={(e) => handleChange('pension.employerPercent', parseInt(e.target.value))} />
              </div>
            </div>
          </div>

          <div className="bg-slate-50 dark:bg-slate-900/20 p-5 rounded-xl border border-slate-100 dark:border-slate-700">
            <h2 className="text-[11px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest mb-4">Rounding Rules</h2>
            <div className="flex gap-3">
              ...
              <div className="w-24">
                <label className="text-[10px] font-bold text-slate-500 mb-1 block">Decimals</label>
                <input type="number" value={taxData.rounding.precision} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 p-2 rounded-lg text-center font-black" onChange={(e) => handleChange('rounding.precision', parseInt(e.target.value))} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {taxData.statutoryDeductions.map((d, idx) => (
              <div key={idx} className="flex gap-3 bg-white dark:bg-slate-900/60 p-3 rounded-lg border border-slate-100 dark:border-slate-700 shadow-sm">
                <input placeholder="Deduction Name" value={d.name} className="flex-1 bg-transparent border-b border-slate-200 dark:border-slate-700 text-sm py-1 outline-none focus:border-emerald-500 dark:text-white" onChange={(e) => updateItem('statutoryDeductions', idx, 'name', e.target.value)} />
                <div className="flex items-center gap-2">
                  <input type="number" placeholder="%" value={d.percent} className="w-16 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 p-1.5 rounded text-center text-sm font-bold" onChange={(e) => updateItem('statutoryDeductions', idx, 'percent', parseInt(e.target.value))} />
                  <button onClick={() => removeItem('statutoryDeductions', idx)} className="text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={16} /></button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {taxData.exemptions.map((ex, idx) => (
              <div key={idx} className="bg-slate-50 dark:bg-slate-900/40 p-4 rounded-xl border border-slate-100 dark:border-slate-700 flex flex-col gap-4">
                <input placeholder="Exemption Label" value={ex.name} className="bg-transparent border-b border-slate-200 dark:border-slate-700 p-1 font-bold text-sm outline-none focus:border-blue-500 dark:text-white" onChange={(e) => updateItem('exemptions', idx, 'name', e.target.value)} />
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-slate-400">LIMIT:</span>
                    <input type="number" value={ex.limit} className="w-24 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 p-1 rounded text-xs font-bold" onChange={(e) => updateItem('exemptions', idx, 'limit', parseInt(e.target.value))} />
                  </div>
                  <label className="flex items-center gap-2 text-[10px] font-black text-slate-500 cursor-pointer uppercase">
                    <input type="checkbox" className="accent-blue-500" checked={ex.overtimeTaxable} onChange={(e) => updateItem('exemptions', idx, 'overtimeTaxable', e.target.checked)} />
                    OT Taxable
                  </label>
                </div>
                <button onClick={() => removeItem('exemptions', idx)} className="text-[9px] text-red-500 font-bold uppercase self-end hover:bg-red-50 p-1 px-2 rounded">Remove Item</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4 mb-10">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {taxData.compliance.map((c, idx) => (
              <div key={idx} className="flex flex-col gap-1 p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm">
                <input placeholder="LABEL" value={c.label} className="text-[9px] uppercase font-black text-slate-400 dark:text-slate-500 bg-transparent outline-none" onChange={(e) => updateItem('compliance', idx, 'label', e.target.value)} />
                <div className="flex items-center gap-2">
                  <input placeholder="Value" value={c.value} className="flex-1 text-xs font-bold bg-transparent outline-none focus:text-blue-600 dark:text-blue-400" onChange={(e) => updateItem('compliance', idx, 'value', e.target.value)} />
            ...
            <div className="flex flex-col items-center">
              <button
                onClick={(e) => {
                  ...
                  setAllTaxCodes((prev) =>
                    prev.map((c) =>
                      c.id === code.id ? { ...c, isEnabled: !c.isEnabled } : c
                    )
                  );
                }}
                className={`w-10 h-5 flex items-center rounded-full p-1 transition-all duration-300 ${
                  code.isEnabled ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'bg-slate-300 dark:bg-slate-600'
                }`}
              >
                <div
  ...
  <div className="flex h-full w-full flex-col gap-4 scrollbar-hidden overflow-y-auto">
    ...
    <div className="bg-gray-50 dark:bg-slate-700 p-6 rounded shadow dark:shadow-black dark:inset-shadow-xs dark:inset-shadow-slate-600 flex flex-col md:flex-row items-center justify-between gap-4 transition-colors">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100">{code.name}</h1>
          <p className="text-slate-400 dark:text-slate-500 text-xs font-semibold uppercase tracking-wider">Configuration History</p>
        </div>
      </div>
    ...
    <div className="flex flex-col gap-3 px-1">
      {code.versions.map((v, i) => (
        ...
        >
          {/* VERSION ROW HEADER */}
          <div className="p-5 flex justify-between items-center cursor-pointer" onClick={() => setExpandedVersionIndex(expandedVersionIndex === i ? null : i)}>
            <div className="flex items-center gap-6">
              <div className="text-lg font-black text-slate-300 dark:text-slate-600 tracking-tighter w-12">{v.version}</div>
              <div className="flex flex-col">
                <span className="text-xs font-bold text-slate-700 dark:text-slate-200">
                  {v.validFrom} {v.validTo ? `to ${v.validTo}` : '(Current)'}
                </span>
                <div className={`mt-1 flex items-center gap-1 w-fit px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-widest border ${
            ...
            <div className="flex items-center gap-4">
              <button 
                ...
                className={`w-10 h-5 flex items-center rounded-full p-1 transition-all ${
                  !code.isEnabled ? 'opacity-20 bg-slate-400' : v.status.active ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'
                }`}
              >
                <div className={`bg-white w-3 h-3 rounded-full shadow transition-transform ${v.status.active && code.isEnabled ? 'translate-x-5' : ''}`} />
              </button>
              <ChevronDown size={18} className={`text-slate-400 transform transition-transform ${expandedVersionIndex === i ? 'rotate-180' : ''}`} />
            </div>
          </div>
      ...
      <div className="bg-amber-50 dark:bg-slate-800 overflow-hidden shadow-sm dark:shadow-black dark:border dark:border-slate-600 rounded-sm">
        {v.incomeTax.type === 'flat' ? (
          ...
          v.incomeTax.brackets.map((b, bi) => (
            <div key={bi} className="flex shadow justify-between p-2 text-xs border-b-2 last:border-0 border-white dark:border-slate-700">
              <span className="text-slate-500 dark:text-slate-400 font-medium">
                {b.min.toLocaleString()} - {b.max ? b.max.toLocaleString() : '∞'}
              </span>
              <span className="font-bold text-blue-600 dark:text-blue-400">{b.rate}%</span>
            </div>
          ))
        )}
      </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="bg-white dark:bg-slate-800 p-3 rounded border border-slate-300 dark:border-slate-600 text-xs flex justify-between shadow-sm">
          <span className="capitalize text-slate-700 dark:text-slate-300">{v.rounding.method}</span>
          <span className="font-bold text-slate-900 dark:text-slate-100">{v.rounding.precision} Decimals</span>
        </div>
      </div>
    </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {v.exemptions.map((ex, exi) => (
            <div key={exi} className="bg-white dark:bg-slate-800 p-3 py-1.5 rounded border border-slate-200 dark:border-slate-600 text-[11px] shadow-sm">
              ...
              <p className="text-slate-400 dark:text-slate-500 mt-0.5 uppercase font-bold tracking-tighter">
                Limit: {ex.limit} | {ex.overtimeTaxable ? 'OT Taxable' : 'OT Exempt'}
              </p>
            </div>
          ))}
        </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-900/40 p-4 border border-transparent dark:border-slate-800 rounded-lg">
        ...
        <div className="space-y-1">
          {v.statutoryDeductions.map((d, di) => (
            <div key={di} className="text-xs flex justify-between font-bold text-slate-600 dark:text-slate-400">
              <span>{d.name}</span>
              <span className="text-slate-800 dark:text-slate-200">{d.percent}%</span>
            </div>
          ))}
          {v.compliance.map((c, ci) => (
            <div key={ci} className="text-[11px] flex justify-between text-slate-400 pt-1 border-t border-white dark:border-slate-800 mt-1">
              <span>{c.label}</span>
              <span className="text-slate-600 dark:text-slate-300">{c.value}</span>
            </div>
          ))}
        </div>
      </div>
