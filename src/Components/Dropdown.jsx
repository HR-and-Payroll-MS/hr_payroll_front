    ...
    return(
        <div className="relative w-full" ref={dropdownRef}>
            <button
                ...
                className="w-full flex items-center justify-between big white border border-gray-300 rounded-lg px-4 py-2  hover:border-gay-400 transition"
            >
            ...
            { isOpen && (
                ...
                {options.length > 0?(
                    options.map((item, index)=>(
                        <li key={index} onClick={()=>handleSelect(item)} className={`px-4 py-2 cursor-pointer hover:bg-blue-50 ${selected === item ? "bg-blue-100 text-blue-600":""}`}>
                            {item}
