...
const StepPayroll = ({ data, onChange }) => {
    ...
    useEffect(() => {
        ...
        const fetchBanks = async () => {
            try {
                const response = await axiosPrivate.get("/payroll/banks/");
                if (isMounted) {
    ...
    const bankOptions = banks.map(b => b.name); // Simple array for Dropdown
    // Helper to find ID from name
    const handleBankChange = (name) => {
        const bank = banks.find(b => b.name === name);
        onChange({ bank_name: name, bank_id: bank ? bank.id : null });
    };
        ...
        <div className="bg-white dark:bg-slate-800 rounded shadow dark:shadow-black dark:inset-shadow-xs dark:inset-shadow-slate-600 transition-all border border-slate-100 dark:border-transparent">
            ...
            <div id="left" className="flex gap-5 p-4 justify-start items-start flex-wrap">
                ...
                <div className="w-96 flex gap-2 text-nowrap items-center">
                    ...
                    <input type="text" value={data.employmenttype} onChange={(e) => onChange({ employmenttype: e.target.value })} className="w-full bg-white dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 rounded px-3 py-1.5 outline-none focus:border-emerald-500 transition-all shadow-sm" />
                </div>

                {/* Job Title */}
                <div className="w-96 flex gap-2 text-nowrap items-center">
                    <p className="min-w-40 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">Job Title</p>
                    <input type="text" value={data.jobtitle} onChange={(e) => onChange({ jobtitle: e.target.value })} className="w-full bg-white dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 rounded px-3 py-1.5 outline-none focus:border-emerald-500 transition-all shadow-sm" />
                </div>

                {/* Job Date */}
                <div className="w-96 flex gap-2 text-nowrap items-center">
                    <p className="min-w-40 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">Job Date</p>
                    <input type="date" value={data.jobdate} onChange={(e) => onChange({ jobdate: e.target.value })} className="w-full bg-white dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 rounded px-3 py-1.5 outline-none focus:border-emerald-500 transition-all shadow-sm" />
                </div>
            ...
            <div className="flex mx-4 py-4 border-b dark:border-slate-700 items-center mt-4">
                <p className="flex-1 text-sm font-bold uppercase tracking-wider text-slate-700 dark:text-slate-200">Bank Details</p>
            </div>

            <div className="flex gap-5 p-4 justify-start items-start flex-wrap">
                {/* Bank Selection */}
                <div className="w-96 flex gap-2 text-nowrap items-center">
                    <p className="min-w-40 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">Bank Name</p>
                    <div className="w-full">
                        <Dropdown padding='p-1.5' options={bankOptions} onChange={handleBankChange} selected={data.bank_name} />
                    </div>
                </div>

                {/* Account Number */}
                <div className="w-96 flex gap-2 text-nowrap items-center">
                    <p className="min-w-40 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">Account Number</p>
